...
type Provider interface {
	...
	// EnsureDKIMKey generates (or replaces, on rotation) the signing key the
	// mail core uses for the domain and returns the base64 public key for
	// the DNS TXT record. algorithm is "rsa" or "ed25519". The private key
	// is generated by and stays inside the mail core; it never transits the
	// control plane, its database, its logs or its API responses (V4 §22).
	EnsureDKIMKey(ctx context.Context, domain, selector, algorithm string) (publicKey string, err error)
}
...
func (Disabled) EnsureDKIMKey(context.Context, string, string, string) (string, error) {
	return "", nil
}
